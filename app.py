...
if page == "📊 Overview":
    ...
    pct_highbp   = round(df["HighBP"].mean() * 100, 1)
    ...
    c5.metric("High BP Prevalence",f"{pct_highbp}%")
...
elif page == "⚖️  BMI Risk Analysis":

    st.title("⚖️ BMI Risk Analysis")
    st.markdown("---")
...
elif page == "⚠️  High Risk Population":

    st.title("⚠️ High Risk Population Analysis")
    st.markdown("---")
...
# --------------------------------------------------
    ...
    # ANOVA: BMI vs Diabetes groups
    g0 = df[df["Diabetes_012"]==0]["BMI"]
    g1 = df[df["Diabetes_012"]==1]["BMI"]
    g2 = df[df["Diabetes_012"]==2]["BMI"]
    f_stat, p_bmi = f_oneway(g0, g1, g2)

    # Chi-square: Age vs Diabetes
    tbl_age = pd.crosstab(df["Age"], df["Diabetes_012"])
    chi2_age, p_age, _, _ = chi2_contingency(tbl_age)

    # Chi-square: HighBP vs Diabetes
    tbl_bp = pd.crosstab(df["HighBP"], df["Diabetes_012"])
    chi2_bp, p_bp, _, _ = chi2_contingency(tbl_bp)

    results = pd.DataFrame({
        "Test": ["ANOVA (BMI across groups)", "Chi-Square (Age vs Diabetes)", "Chi-Square (High BP vs Diabetes)"],
        "Statistic": [f"F = {f_stat:.1f}", f"χ² = {chi2_age:.1f}", f"χ² = {chi2_bp:.1f}"],
        "P-Value": [f"{p_bmi:.2e}", f"{p_age:.2e}", f"{p_bp:.2e}"],
        "Result": ["Reject H₀ ✅", "Reject H₀ ✅", "Reject H₀ ✅"],
        "Interpretation": [
            "BMI significantly differs across diabetes groups",
            "Age category is strongly associated with diabetes",
            "High BP is strongly associated with diabetes",
        ]
    })
    st.dataframe(results, use_container_width=True, hide_index=True)
    st.caption("All tests significant at p < 0.001. Correlation ≠ causation — see findings page.")
...
# --------------------------------------------------
    ...
    st.success(
        ...
        2. **Age** — Diabetes prevalence rises steeply from age 35. Over **60% of diabetic patients are aged 55+**.
        ...
        5. **General Health** — Strongest single correlate (r = 0.30). Patients rating health as "Poor" are overwhelmingly diabetic or pre-diabetic.

        6. **Income & Education** — Negatively correlated with diabetes (r = −0.17 and −0.13). Socioeconomic factors are significant protective factors.
    ...
    st.info(
        ...
        **Data & Research:**
        - Conduct longitudinal studies to establish causal pathways, not just correlations
        - Collect actual age and BMI rather than categorical encodings for future surveys
        - Include dietary quality scores beyond binary fruit/vegetable flags
        """
    )
